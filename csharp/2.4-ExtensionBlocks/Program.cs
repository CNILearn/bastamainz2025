...
static async Task DemonstrateExtensionProperties()
{
    ...
    Rectangle rectangle = new(10.0, 6.0);
...
static async Task DemonstrateUserDefinedOperators()
{
    ...
    // Compound assignment demonstration (simulated since it's not implemented in RC1)
    Complex c3 = new (2, 3);
    ...
    Console.WriteLine($"   Dot Product v1·v2 = {v1 * v2:F2}");
    Console.WriteLine($"   Cross Product v1×v2 = {v1 ^ v2}");
    Console.WriteLine($"   v1 projected onto v2 = {v1.ProjectOnto(v2)}");

    Vector3D normal = new(0, 1, 0); // Y-axis
